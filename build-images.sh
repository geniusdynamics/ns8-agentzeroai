...
app_version="v0.9.8.3"
# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-agentzeroai container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-agentzeroai; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-agentzeroai -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
